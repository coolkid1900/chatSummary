...
用法：
    python -m app.api.seed_data --date 2026-06-27 --customers 300
"""
...
def seed_data(date_str: str, num_customers: int = 300, seed: int | None = None) -> dict:
    """生成并覆盖指定日期的模拟聊天数据，返回写入统计。"""
    if num_customers < 1:
        raise ValueError("customers 必须大于 0")
    if seed is not None:
        random.seed(seed)

    msgs = gen_messages(date_str, num_customers)
    ...
    try:
        ...
        for tbl in all_shard_tables():
            session.execute(text(f"DELETE FROM `{tbl}` WHERE create_date = :d"), {"d": date_str})
        # 分表批量插入
    ...
    return {
        ...
        "messages": len(msgs),
        "shards": len(by_shard),
        "customer_text_messages": len(cust_texts),
        "unique_customer_text_messages": uniq,
    }
...
def main() -> None:
    ...
    result = seed_data(args.date, args.customers, args.seed)
    print(
        "[seed] 写入 {messages} 条消息到 {shards} 张分表 "
        "(date={date}, customers={customers}); 客户文本 {customer_text_messages} 条, "
        "其中唯一 {unique_customer_text_messages} 条".format(**result)
    )
